...
namespace Cdm.Authentication.OAuth2
{
    ...
    {
        public readonly AuthorizationCodeFlow client;
        ...
        {
            return client is IUserInfoProvider;
        }

        public async Task<AuthenticationHeaderValue> GetAuthenticationHeaderAsync()
        {
            var tokenResponse = await client.GetOrRefreshTokenAsync();
...
#endif
                    return await client.ExchangeCodeForAccessTokenAsync(browserResult.redirectUrl, cancellationToken);
                }
        ...
        {
            return await client.GetOrRefreshTokenAsync(cancellationToken);
        }
        ...
        {
            return await client.RefreshTokenAsync(cancellationToken);
        }
        ...
        {
            return await client.RefreshTokenAsync(refreshToken, cancellationToken);
        }
        ...
        public async Task<IUserInfo> GetUserInfoAsync(CancellationToken cancellationToken = default)
        {
            if (SupportsUserInfo())
            {
                return await ((IUserInfoProvider)client).GetUserInfoAsync(cancellationToken);
            }
